...
from hangman.invalid_operation_exceprion import InvalidOperationException
...
class Game:
    ...
    def __init__(self, allowed_misses: int = 6):
        if allowed_misses < 5 or allowed_misses > 8:
            raise ValueError("Number of allowed misses must be between 5 and 8")
    ...
    def guess_letter(self, letter: str) -> Iterable[str]:
        if self.tries_counter == self.allowed_misses:
            raise InvalidOperationException(f'Exceeded the max number misses number. Allowed {self.allowed_misses}')
